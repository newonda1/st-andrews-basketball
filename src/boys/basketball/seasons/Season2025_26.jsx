        ...
        {seasonTotals.length === 0 ? (
          ...
          <div className="overflow-x-auto px-1">
            <table className="w-full border text-center text-xs sm:text-sm md:text-base table-auto whitespace-nowrap">
              {/* ✅ Change 1: lighter header */}
              <thead className="bg-gray-100 text-gray-900">
                <tr>
                  {/* Sticky Player header with a subtle “seal” like FullCareerStats */}
                  <th
                    className="sticky left-0 z-30 bg-gray-100 border px-2 py-2 text-left cursor-pointer border-r border-gray-300 shadow-[4px_0_0_0_rgba(0,0,0,0.04)]"
                    onClick={() => handleSort("name")}
                  >
                    Player{sortArrow("name")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("jersey")}>
                    #{sortArrow("jersey")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("GamesPlayed")}>
                    GP{sortArrow("GamesPlayed")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("Points")}>
                    PTS{sortArrow("Points")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("Rebounds")}>
                    REB{sortArrow("Rebounds")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("Assists")}>
                    AST{sortArrow("Assists")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("Turnovers")}>
                    TO{sortArrow("Turnovers")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("Steals")}>
                    STL{sortArrow("Steals")}
                  </th>

                  <th className="border px-2 py-2 cursor-pointer" onClick={() => handleSort("Blocks")}>
              ...
              <tbody>
                {/* ✅ Change 2: alternating row colors + sticky cell background matches row */}
                {sortedSeasonTotals.map((player, index) => {
                  ...
                  return (
                    <tr key={player.PlayerID} className={`border-t ${rowBg}`}>
                      {/* ✅ Sticky Player cell (keeps photo + link) */}
                      <td
                        className={`sticky left-0 z-20 ${rowBg} border px-2 py-1 text-left align-middle min-w-[220px] border-r border-gray-300 shadow-[4px_0_0_0_rgba(0,0,0,0.04)]`}
                      >
