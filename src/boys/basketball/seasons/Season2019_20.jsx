...
const SEASON_ID = 2019;

function Season2019_20() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [gamesData, statsData, playersData, bracketsJson, rostersData, schoolsData] =
        await Promise.all([
          gamesRes.json(),
          statsRes.json(),
          playersRes.json(),
  ...
  const seasonTotals = useMemo(() => {
    ...
    return Array.from(totals.values())
      ...
      .sort((a, b) => {
        const jerseyA = getRosterJerseyNumber(rosterEntries, a.PlayerID) ?? 999;
        const jerseyB = getRosterJerseyNumber(rosterEntries, b.PlayerID) ?? 999;
        return Number(jerseyA) - Number(jerseyB);
      });
  }, [playerStats, rosterEntries]);

  const leaders = useMemo(() => {
    const byStat = (key) =>
      [...seasonTotals].sort((a, b) => Number(b[key] || 0) - Number(a[key] || 0))[0] || null;
  ...
  return (
    <div className="pt-2 pb-4 space-y-8 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold text-center mb-2">2019-20 Season</h1>
      ...
      <section className="space-y-4">
        ...
        <div className="text-gray-800 leading-relaxed">
          <p className="mb-3 leading-relaxed">
            The 2019-20 St. Andrew's boys basketball team put together one of the
            program's strongest postseason runs under head coach Mel Abrams Jr.,
            finishing {seasonSummary.wins}-{seasonSummary.losses} overall and{" "}
            {seasonSummary.regionWins}-{seasonSummary.regionLosses} in regular-season
            region play. MaxPreps listed the Lions third in GISA AA Region 2, and
            the team built momentum early with an 11-0 start before the middle of
            January.
          </p>

          <p className="mb-3 leading-relaxed">
            St. Andrew's opened the region tournament with a 70-45 win over Twiggs
            Academy before falling to Robert Toombs Christian Academy in the region
            semifinal. The Lions responded with a state tournament surge, beating
            Solid Rock Academy 51-41, Edmund Burke Academy 49-27, and Memorial Day
            49-38 to reach the state championship game.
          </p>

          <p className="mb-3 leading-relaxed">
            The season ended as a GISA AA state runner-up finish after a 57-52 loss
            to Furtah Prep. Reggie Sheppard led the Lions in scoring, assists, and
            steals, while Kegan Kurinij anchored the glass as the team's leading
            rebounder. The MaxPreps stat leaders were {leaderText(leaders.points, "Points")}{" "}
            in points, {leaderText(leaders.rebounds, "Rebounds")} in rebounds,{" "}
            {leaderText(leaders.assists, "Assists")} in assists, and{" "}
            {leaderText(leaders.steals, "Steals")} in steals.
          </p>
        </div>
      </section>
