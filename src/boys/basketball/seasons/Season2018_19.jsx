...
const SEASON_ID = 2018;

function Season2018_19() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [gamesData, statsData, playersData, rostersData, schoolsData] = await Promise.all([
        gamesRes.json(),
        statsRes.json(),
        playersRes.json(),
        rostersRes.json(),
      ...
      setRosterEntries(getRosterEntriesForSeason(rostersData, "2018-19"));
    }
  ...
  const seasonTotals = useMemo(() => {
    ...
    return Array.from(totals.values())
      .map((total) => {
        const rosterEntry = rosterByPlayerId.get(Number(total.PlayerID));
        const rosterGamesPlayed = Number(rosterEntry?.GamesPlayed);
        return {
          ...total,
          GamesPlayed: Number.isFinite(rosterGamesPlayed)
            ? rosterGamesPlayed
            : total.GamesPlayedSet.size,
        };
      })
      .sort((a, b) => {
        ...
        return nameForSort(a.PlayerID).localeCompare(nameForSort(b.PlayerID));
      });
  }, [playerStats, rosterByPlayerId, rosterEntries, playerById]);
  ...
  return (
    <div className="pt-2 pb-4 space-y-8 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold text-center mb-2">2018-19 Season</h1>
      ...
      <section className="space-y-4">
        ...
        <div className="rounded-lg border bg-white p-4 text-gray-800 leading-relaxed shadow-sm">
          <p>
            St. Andrew's finished {seasonSummary.wins}-{seasonSummary.losses} under
            head coach Mel Abrams Jr. The Lions went {seasonSummary.regionWins}-
            {seasonSummary.regionLosses} in regular-season region play, reached the
            region tournament championship game, and advanced to the GISA AA state
            Final Four after state tournament wins over Gatewood and Piedmont Academy.
          </p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-lg border bg-white p-4 shadow-sm">
            ...
            <div className="text-2xl font-bold text-gray-900">Final Four</div>
          </div>
        </div>
      </section>
                ...
                return (
                  <tr key={game.GameID} className={index % 2 ? "bg-gray-50" : "bg-white"}>
                    ...
                    {!showTeamTotals ? (
                      ...
                      <>
                        <td className="border px-3 py-2 text-center">{totals.Points ?? game.TeamScore}</td>
                        <td className="border px-3 py-2 text-center">{totals.Rebounds ?? "-"}</td>
                        <td className="border px-3 py-2 text-center">{totals.Assists ?? "-"}</td>
                        <td className="border px-3 py-2 text-center">{totals.Turnovers ?? "-"}</td>
                        <td className="border px-3 py-2 text-center">{totals.Steals ?? "-"}</td>
                        <td className="border px-3 py-2 text-center">{totals.Blocks ?? "-"}</td>
                        <td className="border px-3 py-2 text-center">{totals.ThreePM ?? "-"}</td>
                        <td className="border px-3 py-2 text-center">
