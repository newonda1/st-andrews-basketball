...
const SEASON_PITCHING_RATE_MIN_OUTS = 15 * 3;
    ...
    const run = async () => {
      try {
        ...
        for (const s of playerStatsData) {
          ...
          const mapKey = `${s.PlayerID}__${seasonKey}`;

          if (!seasonTotalsMap.has(mapKey)) {
            seasonTotalsMap.set(mapKey, {
          ...
          const totals = seasonTotalsMap.get(mapKey);
        ...
        for (const def of recordDefs) {
          const list = seasonTotals
            .map((seasonRow) => {
              const player = playerMap.get(String(seasonRow.PlayerID));
              return {
                value: def.valueFn(seasonRow),
                playerId: String(seasonRow.PlayerID),
