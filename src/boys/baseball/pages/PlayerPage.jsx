...
function addStats(total, row) {
  ...
  total[1] += safeNumber(row["1B"]);
  total[2] += safeNumber(row["2B"]);
  total[3] += safeNumber(row["3B"]);
...
const STAT_VIEWS = {
  batting: {
    ...
    summaryColumns: [
      ...
      { key: "2B", label: "2B", render: (s) => s[2] },
      { key: "3B", label: "3B", render: (s) => s[3] },
    ...
    logColumns: [
      ...
      { key: "2B", label: "2B", render: (g) => g.stats[2] },
      { key: "3B", label: "3B", render: (g) => g.stats[3] },
