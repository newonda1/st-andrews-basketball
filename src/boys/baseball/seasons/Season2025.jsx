  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const seasonPlayerStats = (Array.isArray(statsData) ? statsData : []).filter((s) => {
        if (!seasonGameIds.has(Number(s.GameID))) return false;

        const statId = s?.StatID == null ? "" : String(s.StatID);
        if (!statId) return true;
        if (seenStatIds.has(statId)) return false;
