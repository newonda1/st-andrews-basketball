...
function normalizePlayerGameRow(rawRow, playersById, seasonMap, gamesById) {
  ...
  return {
    ...
    OpponentID: String(rawRow?.OpponentID || matchedGame?.OpponentID || "").trim(),
...
function aggregateTeamGames(games, playerGameRows, seasonMap) {
  ...
  (games || []).forEach((game) => {
    ...
    gameMap.set(gameId, {
      ...
      OpponentLogoPath: String(game?.OpponentLogoPath || ""),
  ...
  playerGameRows.forEach((row) => {
    ...
    if (!gameMap.has(key)) {
      gameMap.set(key, {
        ...
        OpponentLogoPath: String(row?.OpponentLogoPath || ""),
