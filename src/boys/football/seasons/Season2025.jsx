            ...
            <tr>
              {columns.map((column) => (
                <th
                  ...
                  className="px-2 py-2 text-xs text-center whitespace-nowrap"
                >
                  {column.label}
                </th>
              ))}
...
function CombinedTeamStatsTable({ sections }) {
  return (
    <>
      <div className="overflow-x-auto lg:hidden rounded-lg border border-gray-200 bg-white shadow">
        <table className="min-w-full bg-white text-sm">
          <thead className="bg-gray-100 text-gray-700">
            <tr>
              <th className="px-3 py-2 text-left text-xs uppercase tracking-wide">Metric</th>
...
function formatScore(game) {
  if (!Number.isFinite(Number(game?.TeamScore)) || !Number.isFinite(Number(game?.OpponentScore))) {
    return "—";
  }
  return `${game.Result || ""} ${game.TeamScore}–${game.OpponentScore}`.trim();
}
...
export default function Season2025() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (cancelled) return;
        setStatus(error?.message || "Failed to load the 2025 football season.");
      }
    }
  ...
  const rosterSeason = useMemo(
    () => rosters.find((entry) => Number(entry.SeasonID) === SEASON_ID) || null,
    [rosters]
  );
  ...
  const seasonGames = useMemo(
    () =>
      sortGamesChronologically(
        games.filter((game) => Number(game.SeasonID ?? game.Season) === SEASON_ID)
      ),
    [games]
  );
  ...
  const passingTable = useMemo(
    () => getSeasonStatTable(seasonStats, "Passing"),
    [seasonStats]
  );
  const rushingTable = useMemo(
    () => getSeasonStatTable(seasonStats, "Rushing"),
    [seasonStats]
  );
  ...
  const tacklesTable = useMemo(
    () => getSeasonStatTable(seasonStats, "Tackles"),
    [seasonStats]
  );
  const sacksTable = useMemo(
    () => getSeasonStatTable(seasonStats, "Sacks"),
    [seasonStats]
  );
  ...
  const puntsTable = useMemo(
    () => getSeasonStatTable(seasonStats, "Punts"),
    [seasonStats]
  );
  ...
  const offenseRows = useMemo(
    () => [
      ...
      { label: "Receiving yards", value: receivingTable?.Totals?.receivingyards || "—" },
      {
        label: "Total offense",
        value:
          totalYardsTable?.Totals?.totalyards && totalYardsTable?.Totals?.totalyardspergame
      ...
      },
      {
        ...
        value:
          ...
          allPurposeTable?.Totals?.allpurposeyardspergame
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-4 pb-10 pt-2 lg:pb-40">
      ...
      <section className="space-y-2 text-center">
        <h1 className="text-3xl font-bold">2025-26 Season</h1>
      </section>
