...
function ArticleImageGallery({ images, title }) {
        ...
        {normalizedImages.map((image) => (
          <figure key={image.src} className="mx-auto w-full max-w-5xl space-y-2">
            <button
              ...
              className="block w-full cursor-zoom-in border border-gray-200 bg-white p-0 text-left shadow-sm transition hover:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
          ...
          <div className="fixed left-0 right-0 top-0 z-[101] flex items-center justify-between gap-3 border-b border-white/20 bg-black/90 px-3 py-2">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => setZoom(100)}
                className="h-10 border border-white/30 px-3 text-sm font-bold"
              ...
              <button
                type="button"
                onClick={() => setZoom((value) => Math.min(400, value + 50))}
                className="h-10 w-10 border border-white/30 text-lg font-bold"
                aria-label="Zoom in"
                title="Zoom in"
              >
