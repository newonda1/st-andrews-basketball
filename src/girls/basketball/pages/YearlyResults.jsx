...
function YearlyResults() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/data/girls/basketball/games.json").then((res) => res.json()),
      fetch("/data/girls/basketball/seasons.json").then((res) => res.json()),
      fetch("/data/girls/basketball/seasonsadjustments.json")
          ...
          <tbody>
            {seasonStats.map((season) => (
              <tr key={season.seasonKey}>
                <td className={numericCellClassName}>
                  <Link
                    to={`/athletics/girls/basketball/seasons/${season.routeSlug}`}
