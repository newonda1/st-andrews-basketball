...
function getDefaultCsvPath(projectRoot, gameId) {
  return path.join(projectRoot, "imports/baseball/gamechanger", `${gameId}.csv`);
}

function resolveCsvPath(projectRoot, filePath, gameId) {
  if (filePath) {
    return path.isAbsolute(filePath) ? filePath : path.resolve(projectRoot, filePath);
  }

  if (!gameId) {
    throw new Error("A GameID is required when no CSV path is provided.");
  }

  return getDefaultCsvPath(projectRoot, gameId);
}
...
function loadGameRecord(projectRoot, gameId) {
  const { GAMES_PATH } = getProjectPaths(projectRoot);
  const games = JSON.parse(fs.readFileSync(GAMES_PATH, "utf-8"));
  return games.find((g) => Number(g.GameID) === Number(gameId)) || null;
}

function describeGameRecord(gameRecord) {
  if (!gameRecord) return "No matching game found in games.json";

  const parts = [];
  if (gameRecord.Season != null) parts.push(`Season ${gameRecord.Season}`);
  if (gameRecord.Opponent) parts.push(`vs ${gameRecord.Opponent}`);
  if (gameRecord.LocationType) parts.push(`(${gameRecord.LocationType})`);
  return parts.join(" ");
}

function importGameChangerCsv({ projectRoot = process.cwd(), gameId, filePath, write = true }) {
  if (!gameId) {
    ...
    if (gameRecord.LineScore?.OpponentTotals?.H != null) {
      validationSummary.push(
        formatValidationLine(
          ...
          generatedTotals.hitsAllowed,
          gameRecord.LineScore.OpponentTotals.H
        )
      );
    }
  }
  ...
  const filteredExistingStats = existingStats.filter((row) => Number(row.GameID) !== Number(gameId));
  ...
  return {
    ...
    filePathRelative: path.relative(projectRoot, resolvedFilePath) || path.basename(resolvedFilePath),
    statsPath: paths.STATS_PATH,
    parsedRowCount: parsed.length,
    playerRowCount: dataRows.length,
    matchedRowCount: generatedRows.length,
    unmatchedRows,
    pitcherRowCount: pitcherRows.length,
    generatedTotals,
    validationSummary,
    generatedRows,
    generatedStats,
    gameRecord,
    existingRowsRemoved: existingStats.length - filteredExistingStats.length,
    updatedStatsCount: updatedStats.length,
    wroteFile: write,
  };
}

function printImportSummary(result, options = {}) {
  const {
    log = console.log,
    showPreview = true,
    showFullJson = false,
  ...
  log("GameID:", result.gameId);
  log("CSV file:", result.filePathRelative);
  log("Game:", describeGameRecord(result.gameRecord));
  log("Total raw rows:", result.parsedRowCount);
  log("Player rows found:", result.playerRowCount);
  log("Matched player rows:", result.matchedRowCount);
  log("Unmatched rows:", result.unmatchedRows.length);
  log("Pitchers with IP > 0:", result.pitcherRowCount);
  log("Generated totals:", result.generatedTotals);
  log("Existing rows removed for this GameID:", result.existingRowsRemoved);

  if (result.unmatchedRows.length) {
    log("\nUnmatched players:\n");
    log(result.unmatchedRows);
  }

  if (result.validationSummary.length) {
    log("\nValidation summary:\n");
    result.validationSummary.forEach((item) => log(item));
  }

  if (showPreview) {
    log("\nGenerated row preview:\n");
    result.generatedRows.slice(0, 5).forEach((entry, idx) => {
      log(idx + 1, {
        jersey: entry.jersey,
  ...
  if (showFullJson) {
    ...
    log(JSON.stringify(result.generatedStats, null, 2));
  }

  if (result.wroteFile) {
    log("\nplayergamestats.json updated successfully.\n");
    log(`Replaced rows for GameID ${result.gameId} with ${result.generatedStats.length} imported rows.`);
  } else {
    log("\nDry run complete. playergamestats.json was not changed.\n");
    log(`Would replace rows for GameID ${result.gameId} with ${result.generatedStats.length} imported rows.`);
  }
}
